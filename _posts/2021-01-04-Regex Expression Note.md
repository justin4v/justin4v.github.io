...
> **正向**：是指 找到 **匹配 pattern** 的内容
>
> **负向**：是指 找到 **不匹配 pattern** 的内容
>
> **先行**：是指 **expression 在匹配 pattern 的内容前面查找匹配**
>
> **后行**：是指 **expression 在匹配 pattern 的内容后面查找匹配**
...
>
> 作用：**返回 pattern 所匹配内容的前面内容中匹配 expression 的内容，不返回本身**。
>
> 含义：**断定匹配 pattern 的内容前面存在匹配 expression 的内容，并返回该内容（不包含pattern 匹配的内容）**
...
>
> 作用：**返回 pattern 所匹配内容的后面内容中匹配 expression 的内容，不返回本身**
>
> 含义：**断定匹配 pattern 的内容后面存在匹配 expression 的内容，并返回该内容（不包含pattern 匹配的内容）**
...
**后行断言要加上 “<” 符号==(开口向后，表明 pattern 向后寻找 expression)==；负向需加上！**
